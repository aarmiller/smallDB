...
build_time_map_keys <- function(collect_tab=collect_table(), db_con){
  ...
  if (any(collect_tab$setting=="outpatient")) {
    ...
    temp.out <- collect_tab %>%
      ...
      gether_core_data(vars = c("enrolid","stdplac","svcdate"),db_con = db_con) %>%
  ...
  out_keys <- temp_time_map %>%
    ...
    dplyr::select("year","ccae","enrolid","stdplac","svcdate"="admdate","key")

  # get distinct inpatient keys
  in_keys <- temp_time_map %>%
    dplyr::filter(.data$inpatient==1) %>%
    dplyr::select("year","ccae","enrolid","admdate","disdate","caseid","key")

  # get distinct rx keys
  rx_keys <- temp_time_map %>%
    dplyr::filter(.data$inpatient==0 & .data$rx==1) %>%
    dplyr::select("year","ccae","enrolid","svcdate"="admdate","key")
...
build_time_map <- function(db_con,collect_tab=collect_table()){
  if (!any(DBI::dbListTables(db_con) %in% c("outpatient_keys","inpatient_keys"))){
    ...
    add_time_map_keys(collect_tab=collect_table(),db_con = db_con,temporary = TRUE)
  }
  ...
  dat <- rbind(db_con %>%
                 ...
                 dplyr::mutate(disdate=.data$svcdate,
                               inpatient=0L,
                               rx=0L) %>%
                 dplyr::select("key","year","ccae","enrolid","admdate"="svcdate","disdate","inpatient","rx","stdplac"),
